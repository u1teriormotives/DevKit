...
// i don't readily have access to a windows machine, so i cannot guarantee this
// will work
...
__declspec(dllimport) NTSTATUS __stdcall
    BCryptGenRandom(void* hAlgorithm, unsigned char* pbBuffer,
...
static void rng_bytes(void* buf, size_t n) {
  ...
  while (n > 0) {
    ...
    NTSTATUS st =
        BCryptGenRandom(NULL, p, chunk, BCRYPT_USE_SYSTEM_PREFERRED_RNG);
    if (st != 0) {
